...
function set_types_db {
	...
	for i in `mysql -N -u${dbuser} -p${dbpass} -e "${node_query}" ${db_weather}` ; do
	if !(snmpget -v2c -c $(get_snmp $i) $(get_hostname $i) ${nsm_oid_host} | grep "No\ Such|Timeout" &>2) ; then
		echo $(get_hostname $i)
	fi
	done
}
...
function check_coord {
	local host_id=$1
	local coord=$2
	coord_val=( $(get_coord ${host_id} ${coord}) )
	if [ -z ${coord_val} ] ; then
		echo ${coord_val}
	else 
		coord_val=
...
# set_coord - Takes specification of x or y coord Prompts user for coordinate value
function set_coord {
	local host_id=$1
	local coord=$2
	local new_value=$3
